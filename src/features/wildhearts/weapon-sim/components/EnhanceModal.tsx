...
export default function EnhanceModal(): JSX.Element | null {
  ...
  if (Boolean(coord)) {
    return (
      <>
        <div onClick={() => close('enhanceModal')} className="fixed text-xs md:text-sm bg-gray-800 bg-opacity-60 top-0 left-0 right-0 z-50 w-full pt-10 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] md:h-full">
          <div className="relative w-full h-full md:h-auto">
            <div className="relative">
              <div className="px-4 py-2 space-y-2 grid grid-cols-1 min-[390px]:grid-cols-[1fr_24px_1fr] sm:grid-cols-[1fr_128px_1fr]">
                <div className="place-self-center min-[390px]:place-self-end">
                  <table onClick={(e) => { e.stopPropagation() }} className="bg-gray-800 border-separate border-4 border-amber-400 text-gray-100 rounded-lg border-spacing-0 w-40 md:w-52">
                    ...
                    <tr>
                      <td colSpan={2}>
                        <ul>
                          { [0,1,2].map(i => {
                            return currentWeapon?.inherentSkills[i] ?
                              <li key={i} className="bg-gray-900/75 h-6 pt-0.5 mb-1 px-2" data-tooltip-id="skill-enhance-tooltip" data-tooltip-content={currentWeapon.inherentSkills[i]}>{currentWeapon?.inherentSkills[i]}</li> :
                              <li key={i} className="bg-gray-900/75 h-6 pt-0.5 mb-1 px-2">{currentWeapon?.inherentSkills[i]}</li>
                          })}
                        </ul>
                      </td>
                    </tr>
                        ...
                        <ul>
                          { [...Array(5)].map((_, i) => i).map(i => {
                            ...
                            if (skill) {
                              ...
                              return skill.name ?
                                <li key={i} className={classes} onClick={() => selectSkill(skill)} data-tooltip-id="skill-enhance-tooltip" data-tooltip-content={skill.name}>{skill.name}</li> :
                ...
                <div className="place-self-center">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={3} stroke="currentColor" className="w-6 h-6 rotate-90 md:w-24 md:h-24 min-[390px]:rotate-0 text-gray-100 ">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M11.25 4.5l7.5 7.5-7.5 7.5m-6-15l7.5 7.5-7.5 7.5" />
                  </svg>
                </div>

                <div className="place-self-center min-[390px]:place-self-start">
                  <table onClick={(e) => { e.stopPropagation() }} className="bg-gray-800 border-separate border-4 border-amber-400 text-gray-100 rounded-lg border-spacing-0 w-40 md:w-52">
                    <tbody className="text-xs md:text-sm">
                    <tr>
                    ...
                    <tr>
                      <td colSpan={2}>
                        <ul>
                          { [0,1,2].map(i => {
                            return selectedWeapon.inherentSkills[i] ?
                              <li key={i} className="bg-gray-900/75 h-6 pt-0.5 mb-1 px-2" data-tooltip-id="skill-enhance-tooltip" data-tooltip-content={selectedWeapon.inherentSkills[i]}>{selectedWeapon.inherentSkills[i]}</li> :
                              <li key={i} className="bg-gray-900/75 h-6 pt-0.5 mb-1 px-2">{selectedWeapon.inherentSkills[i]}</li>
                          })}
                        </ul>
                      </td>
                    </tr>
                      ...
                      <td colSpan={2}>
                        <ul>
                          { [...Array(5)].map((_, i) => i).map(i => {
                            ...
                            if (skill) {
                              return <li key={i} className={classes} data-tooltip-id="skill-enhance-tooltip" data-tooltip-content={skill.name}>{skill.name}</li>
                            }
                            skill = selectedSkills[i - selectedWeapon.inheritedSkills.length]
                            if (skill) {
                              return skill.name ?
                                <li key={i} className={classes} onClick={() => deSelectSkill(skill)} data-tooltip-id="skill-enhance-tooltip" data-tooltip-content={skill.name}>{skill.name}</li> :
                                <li key={i} className={classes}>{skill.name}</li>
                            }
                ...
                <div className="place-self-center md:place-self-start">
                  <table className="bg-gray-800 border-separate border-2 border-amber-400 text-gray-100 rounded-lg border-spacing-0 w-40 md:w-52">
                    <tbody>
                      { selectedWeapon.materials.map(material => {
                        return (
                          <tr key={material.name} className="py-1">
                            ...
                            <td className="pl-1 pr-3 md:pr-5 border-b border-amber-200 text-right">{material.count}</td>
                          </tr>
                        )
                      }) }
                      <tr className="py-1">
                        <td className="pl-5 pr-1">金</td>
                        <td className="pl-1 pr-3 md:pr-5 text-right">{selectedWeapon.gold}</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
          ...
          <SkillToolTip id="skill-enhance-tooltip" />
        </div>
      </>
    )
  } else {
